...
class Metrics(object):
    ...
    def __init__(self, score_file_path:str):
        ...
        print("self.segment: ", self.segment)
    ...
    def __read_socre_file(self, score_file_path):
        ...
        with open(score_file_path, 'r') as infile:
            print(score_file_path, 'opened')
            i = 0
            for line in infile.readlines():
                print('i: ', i+1, ", Segment: ", self.segment)
                i += 1
                tokens = line.strip().split('\t')
                print("Tokens: ", tokens)
                one_sess.append((float(tokens[0]), int(tokens[1])))
                if i % self.segment == 0:
                    print("i % self.segment == 0")
    ...
    def __mean_average_precision(self, sort_data):
        ...
        for index in range(len(sort_data)):
            print('sort_data[index][1]:', sort_data[index][1])
    ...
    def evaluate_all_metrics(self):
        ...
        sum_r_2 = 0
        sum_r_5 = 0
        print('Why you not printing')
        print('Path in args', self.score_file_path)
        print("self.segment: ", self.segment)
        sessions = self.__read_socre_file(self.score_file_path)
        print('Total Sessions:', len(sessions), 'Sessions: ', sessions)
